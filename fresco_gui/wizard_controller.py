...
"""
    ...
    def _generate_inelastic_input(self, data: Dict[str, Any]) -> str:
        """Generate inelastic scattering input with states and coupling"""
        ...
        coupling_kind = data.get('coupling_kind', 'deformed')
        couplings = data.get('couplings', [])

        # Build header
        proj_name = proj.get('name', 'p')
        targ_name = targ.get('name', '12C')
        header = f"{proj_name}+{targ_name} @ {energy} MeV; Inelastic"
        ...
        fresco_params = {
            ...
            'smats': params.get('smats', 2) if isinstance(params, dict) else 2,
            'xstabl': params.get('xstabl', 1) if isinstance(params, dict) else 1,
            'iblock': params.get('iblock', 2) if isinstance(params, dict) else 2,
        }
        ...
        # Build PARTITION with nex = number of states
        proj_mass = proj.get('mass', 1.0)
        proj_z = proj.get('atomic_number', 1)
        targ_mass = targ.get('mass', 12.0)
        targ_z = targ.get('atomic_number', 6)
        ...
        nex = max(len(states), 1)
        partition = f" &PARTITION namep='{proj_name}' massp={proj_mass} zp={proj_z} "
        partition += f"namet='{targ_name}' masst={targ_mass} zt={targ_z} qval=0.0 nex={nex} pwf=T /\n"

        # Build STATES for each state
        states_text = ""
        if states:
            for i, state in enumerate(states):
                ex_energy = state.get('excitation', 0.0)
                spin = state.get('spin', 0.0)
                parity_str = state.get('parity', '+1')
                band = state.get('band', 0)
                # Determine bandt: positive parity = 1, negative = -1
                bandt = 1 if '+' in str(parity_str) else -1
                copyp = 0 if i == 0 else 1
                states_text += f" &STATES jp={proj_spin} copyp={copyp} bandp=1 ep=0.0 "
                states_text += f"cpot=1 jt={spin} copyt=0 bandt={bandt} et={ex_energy} /\n"
        else:
            states_text = f" &STATES jp={proj_spin} bandp=1 ep=0.0 cpot=1 "
            states_text += f"jt={targ_spin} bandt=1 et=0.0 /\n"
        ...
        if couplings:
            for coupling in couplings:
                from_s = coupling.get('from_state', 1)
                to_s = coupling.get('to_state', 2)
                lam = coupling.get('multipolarity', 2)
                beta = coupling.get('beta', 0.25)
                deform = coupling.get('deform_length', 0.0)
                pot_num = coupling.get('pot_number', 1)

                coupling_text += f" &COUPLING icto={to_s} icfrom={from_s} kind=3 "
                coupling_text += f"ip1={lam} ip2=0 ip3=0 /\n"
                # Deformation parameter
                if deform != 0.0:
                    coupling_text += f"   {deform}\n"
                else:
                    coupling_text += f"   {beta}\n"

        # Assemble
        input_text = f"""{header}
NAMELIST
{fresco_line}
{partition}{states_text} &partition /

{pot_text} &pot /
 &overlap /
{coupling_text} &coupling /
...
"""
    ...
    def populate_from_input_text(self, input_text: str):
        ...
        # Note: mass_number (A) can be inferred from mass (round to nearest int)
        for step in self.step_widgets:
            if hasattr(step, 'step_id') and step.step_id == 'particle_config':
                if partitions:
                    ...
                    if hasattr(step, 'projectile_widget'):
                        ...
                        proj_mass = p1.get('massp', 1.0)
                        pw.mass.setValue(proj_mass)
                    ...
                    if hasattr(step, 'target_widget'):
                        ...
                        targ_mass = p1.get('masst', 12.0)
                        tw.mass.setValue(targ_mass)
            ...
            for step in self.step_widgets:
                if hasattr(step, 'step_id') and step.step_id == 'exit_channel':
                    if hasattr(step, 'ejectile_widget'):
                        ...
                        eject_mass = p2.get('massp', 1.0)
                        ew.mass.setValue(eject_mass)
                    ...
                    if hasattr(step, 'residual_widget'):
                        ...
                        resid_mass = p2.get('masst', 13.0)
                        rw.mass.setValue(resid_mass)
        ...
        if detected_type == ReactionType.INELASTIC and len(states_list) >= 2:
            ...
            for i, st in enumerate(states_list):
                inelastic_states.append({
                    'state_number': i + 1,
                    'excitation': st.get('et', 0.0),
                    'spin': st.get('jt', 0.0),
                    'parity': '+1' if st.get('bandt', 1) > 0 else '-1',
                    'band': abs(st.get('bandt', 1)),
                })
            for step in self.step_widgets:
                if hasattr(step, 'step_id') and step.step_id == 'states':
                    step.set_data({'states': inelastic_states})
                    break
        ...
        if detected_type == ReactionType.TRANSFER:
            ...
            if overlaps:
                # Map parsed overlaps to the format OverlapStep.set_data() expects
                # OverlapStep expects 'projectile_overlap' and 'target_overlap' dicts
                # with keys: binding_energy, radius, diffuseness, rc
                overlap_data = {}
                for ov in overlaps:
                    # First overlap (in=1 or in>0) is projectile overlap
                    # Second overlap (in=2 or in<0) is target overlap
                    in_val = ov.get('in', 1)
                    be = ov.get('be', 0.0)
                    if in_val == 1 or (in_val > 0 and 'projectile_overlap' not in overlap_data):
                        overlap_data['projectile_overlap'] = {
                            'binding_energy': be,
                        }
                    elif in_val == 2 or in_val < 0 or 'projectile_overlap' in overlap_data:
                        overlap_data['target_overlap'] = {
                            'binding_energy': be,
                        }

                for step in self.step_widgets:
                    if hasattr(step, 'step_id') and step.step_id == 'overlap':
                        step.set_data(overlap_data)
                        break
