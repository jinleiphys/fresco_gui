...
class InelasticScatteringForm(QWidget):
    ...
    def load_preset(self):
        """Load preset example for Alpha + 12C inelastic to 2+ state at 4.439 MeV"""
        # Custom default for inelastic scattering
        preset_input = """Alpha + 12C inelastic to 2+ state at 4.439 MeV
NAMELIST
 &FRESCO
     hcm=0.1 rmatch=60.0 jtmax=50.0 absend=0.001 thmin=0.0
     thmax=180.0 thinc=1.0 iter=1 chans=1 smats=2
     xstabl=1 rasym=0.0 accrcy=0.01 jtmin=0.0
      elab=30.000 /

 &PARTITION namep='alpha' massp=4.0 zp=2.0 namet='12C' masst=12.0 zt=6.0 qval=0.0 nex=1  /
 &STATES jp=0.0 bandp=1 ep=0.0 cpot=1 jt=0.0 bandt=1 et=0.0 /
 &partition /

&POT type=0 kp=1 at=78 ap=1 rc=1.2 ac=0 /

&POT type=1 kp=1 shape=0 p1=40 p2=1.2 p3=0.65 p4=10 p5=1.2 p6=0.5 /
...
class TransferReactionForm(QWidget):
    ...
    def load_preset(self):
        """Load preset example from B5-example-tr.in (n14(f17,ne18)c13 transfer reaction)"""
        # Use the exact B5 example file content from test/B5/B5-example-tr.in
        preset_input = """n14(f17,ne18)c13 @ 170 MeV;
NAMELIST
 &FRESCO
     absend=-1.0 centre=0.0 chans=1 hcm=0.03 hnl=0.1
     iblock=0 iter=1 jtmax=120.0 jtmin=0.0 nnu=36
     rintp=0.2 rmatch=40.0 rnl=5.0 smats=2 thinc=0.1
     thmax=40.0 thmin=0.0 xstabl=1 elab=[170.0] nlab=[]
 /

&PARTITION namep='f17' massp=17.0 zp=9.0 namet='n14' masst=14.0 zt=7.0 qval=0.0 nex=2 /
 &STATES jp=2.5 bandp=1 ep=0.0 cpot=1 jt=1.0 bandt=1 et=0.0 /
 &STATES jp=0.0 copyp=1 bandp=1 ep=0.0 cpot=2 jt=0.5 bandt=1 et=0.0 /

&PARTITION namep='p' massp=1.0078 zp=1.0 namet='41Ca' masst=41.0 zt=20.0 qval=5.08 nex=1 /

&STATES jp=0.5 bandp=2 ep=0.0 cpot=2 jt=0.5 bandt=2 et=0.0 /

 &partition /

&POT type=0 kp=1 at=14 ap=17 rc=1.3 ac=0 /

&POT type=1 kp=1 shape=0 p1=37 p2=1.2 p3=0.6 p4=21 p5=1.2 p6=0.69 /

&POT type=0 kp=1 at=13 ap=18 rc=1.3 ac=0 /

&POT type=1 kp=1 shape=0 p1=37 p2=1.2 p3=0.6 p4=21 p5=1.2 p6=0.69 /

&POT type=0 kp=1 at=17 ap=4 rc=1.2 ac=0 /

&POT type=1 kp=1 shape=0 p1=50 p2=1.2 p3=0.65 p4=10 p5=1.2 p6=0.65 /

&POT type=3 kp=1 shape=0 p1=6 p2=1.2 p3=0.65 /

&POT type=0 kp=1 at=13 ap=4 rc=1.2 ac=0 /
...
&OVERLAP kn1=1 ic1=1 ic2=2 in=1 kind=0 ch1='A' nn=1 l=2 sn=0.5 ia=1 jn=0.5 ib=1 kbpot=3 be=3.922 isc=1 ipc=0 /
&OVERLAP kn1=2 ic1=2 ic2=1 in=2 kind=3 ch1='A' nn=1 l=1 sn=0.5 ia=1 jn=0.5 ib=1 kbpot=4 be=7.551 isc=1 ipc=0 /
 &overlap /
 &COUPLING kind=7 icfrom=1 icto=1 ip1=0 ip2=0 /
 &coupling /
...
"""
    ...
    def generate_input(self):
        ...
        # Get NEX (number of excited states) from states manager
        nex = self.states_manager.get_nex()
